...
CE10 = pd.read_excel("/home/andrew/Documents/OOI-CGSN/QAQC_Sandbox/Data_Review/Instrument Review/PCO2W/Endurance-10_SR1813_Discrete_Summary_2018-09-17_ver_1_00.xlsx")
...
# +
# Calculate some key physical parameters to get density based on TEOS-10
SP = CE10[["CTD Salinity 1, uncorrected [psu]","CTD Salinity 1, uncorrected [psu]"]].mean(axis=1)
...
PAR1 = CE10["Discrete DIC [µmol/kg]"]
PAR2 = CE10["Discrete pCO2 [µatm]"]
PAR1TYPE = 2
PAR2TYPE = 4
SAL = CE10["CTD Salinity 1, uncorrected [psu]"]
...
TAlk = pd.DataFrame(data=[CE10["Calculated Alkalinity [µmol/kg]"], CO2dict["TAlk"]], index=["CE","CO2SYS"]).T.dropna()
...
ax.scatter(CE10["Calculated Alkalinity [µmol/kg]"], CO2dict["TAlk"], s=64)
...
basepath = '/home/andrew/Documents/OOI-CGSN/QAQC_Sandbox/QARTOD/PCO2W'

# ---
# #### CP01CNSM MFN

# Load all the deployment datasets
datasets = ['/'.join((basepath, 'CP01CNSM-MFD35-05-PCO2WB000', 'recovered_inst', dset)) for dset in os.listdir('/'.join((basepath, 'CP01CNSM-MFD35-05-PCO2WB000', 'recovered_inst')))]
CNSM_PCO2W = xr.open_mfdataset(sorted(datasets))
CNSM_PCO2W = CNSM_PCO2W.swap_dims({'obs':'time'})
CNSM_PCO2W = CNSM_PCO2W.sortby('time')
# Cut down to one spectrum to reduce the size of the dataset
...
# Load all the deployment datasets
datasets = ['/'.join((basepath, 'CP03ISSM-MFD35-05-PCO2WB000', 'recovered_inst', dset)) for dset in os.listdir('/'.join((basepath, 'CP03ISSM-MFD35-05-PCO2WB000', 'recovered_inst')))]
ISSM_PCO2W = xr.open_mfdataset(datasets)
ISSM_PCO2W = ISSM_PCO2W.swap_dims({'obs':'time'})
ISSM_PCO2W = ISSM_PCO2W.sortby('time')
...
# Load all the deployment datasets
datasets = ['/'.join((basepath, 'CP04OSSM-MFD35-05-PCO2WB000', 'recovered_inst', dset)) for dset in os.listdir('/'.join((basepath, 'CP04OSSM-MFD35-05-PCO2WB000', 'recovered_inst')))]
OSSM_PCO2W = xr.open_mfdataset(datasets)
OSSM_PCO2W = OSSM_PCO2W.swap_dims({'obs':'time'})
OSSM_PCO2W = OSSM_PCO2W.sortby('time')
# Select a single spectrum to reduce the size of the dataset
OSSM_PCO2W = OSSM_PCO2W.sel({'spectrum':0})
OSSM_PCO2W

# ---
# ### Load Associated CTD Data
#

basepath = '/home/andrew/Documents/OOI-CGSN/QAQC_Sandbox/QARTOD/CTDBP'

# #### CP01CNSM MFN

# Load all of the CTDBP datasets
path = '/'.join((basepath, 'CP01CNSM-MFD37-03-CTDBPD000', 'recovered_inst'))
datasets = ['/'.join((path, dset)) for dset in sorted(os.listdir(path))]
CNSM_CTDBP = xr.open_mfdataset(datasets)
CNSM_CTDBP = CNSM_CTDBP.swap_dims({'obs':'time'})
CNSM_CTDBP = CNSM_CTDBP.sortby('time')
CNSM_CTDBP

# #### CP03ISSM MFN

# Load all of the CTDBP datasets
path = '/'.join((basepath, 'CP03ISSM-MFD37-03-CTDBPD000', 'recovered_inst'))
datasets = ['/'.join((path, dset)) for dset in sorted(os.listdir(path))]
ISSM_CTDBP = xr.open_mfdataset(datasets)
ISSM_CTDBP = ISSM_CTDBP.swap_dims({'obs':'time'})
ISSM_CTDBP = ISSM_CTDBP.sortby('time')
ISSM_CTDBP

# #### CP04OSSM MFN

# CP04OSSM Multi-function node 
path = '/'.join((basepath, 'CP04OSSM-MFD37-03-CTDBPE000', 'recovered_inst'))
datasets = ['/'.join((path, dset)) for dset in sorted(os.listdir(path))]
OSSM_CTDBP = xr.open_mfdataset(datasets)
OSSM_CTDBP = OSSM_CTDBP.swap_dims({'obs':'time'})
OSSM_CTDBP = OSSM_CTDBP.sortby('time')
OSSM_CTDBP
...
ax0.scatter(AT27["pCO2 Pressure-corrected"], AT27["Pressure [db]"], c="tab:red")
ax0.grid()
...
ax1.scatter(AT31["pCO2 Pressure-corrected"], AT31["Pressure [db]"], c="tab:blue")
ax1.grid()
