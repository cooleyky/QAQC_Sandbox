...
refdes = "GA01SUMO-RII11-02-CTDBPP031"
...
sorted(os.listdir(f"{inst_dir}/{refdes}"))

method = "recovered_inst"
sorted(os.listdir(f"{inst_dir}/{refdes}/{method}"))

stream = "ctdbp_cdef_instrument_recovered"
sorted(os.listdir(f"{inst_dir}/{refdes}/{method}/{stream}"))
...
ax0.plot(ds.time, ds.ctdbp_seawater_temperature, linestyle="", marker=".", color="tab:red")
ax0.set_ylabel("Temperature")
ax0.grid()

ax1.plot(ds.time, ds.ctdbp_seawater_conductivity, linestyle="", marker=".", color="tab:blue")
ax1.set_ylabel("Conductivity")
ax1.grid()

ax2.plot(ds.time, ds.ctdbp_seawater_pressure, linestyle="", marker=".", color="tab:grey")
...
sys.path.append("/home/andrew/Documents/OOI-CGSN/oceanobservatories/ooi-data-explorations/python/utilities/qartod")
sys.path

# Now import the climatology code
from climatology import calc_regression_climatology, qartod_climatology
...
temp = calc_monthly_means(ds, "ctdbp_seawater_temperature")
sal = calc_monthly_means(ds, "practical_salinity")
pres = calc_monthly_means(ds, "ctdbp_seawater_pressure")
...
while cycles > 0:
    ...
    arr1 = np.sin(cycles*np.pi*f*t)
    arr2 = np.cos(cycles*np.pi*f*t)
...
seasonal_cycle = beta[0] + beta[1]*np.sin(2*np.pi*f*new_t) + beta[2]*np.cos(2*np.pi*f*new_t)# + beta[3]*np.sin(4*np.pi*f*new_t)
# #+ beta[4]*np.cos(4*np.pi*f*new_t)

seasonal_cycle

temp_cycle, temp_beta, temp_sigma = calc_regression_climatology(temp.values.reshape(-1))
sal_cycle, sal_beta, sal_sigma = calc_regression_climatology(sal.values.reshape(-1))
pres_cycle, pres_beta, pres_sigma = calc_regression_climatology(pres.values.reshape(-1))
...
# +
temp_lower = np.round(temp_climatology-temp_sigma*2, decimals=2)
temp_upper = np.round(temp_climatology+temp_sigma*2, decimals=2)

sal_lower = np.round(sal_climatology-cond_sigma*2, decimals=2)
sal_upper = np.round(sal_climatology+cond_sigma*2, decimals=2)

pres_lower = np.round(pres_climatology-pres_sigma*2, decimals=2)
pres_upper = np.round(pres_climatology+pres_sigma*2, decimals=2)

# +
fig, (ax0, ax1, ax2) = plt.subplots(nrows=3, ncols=1, figsize=(15,10))

ax0.plot(ds.time, ds.ctdbp_seawater_temperature, linestyle="", marker=".", color="tab:red")
ax0.plot(temp.time, seasonal_cycle, color="black")
